---
title: "How I Built LinkedIn Automation in n8n + OpenRouter"
date: "2026-06-10"
updated: "2026-06-19"
description: "A two-workflow n8n build: AI writes LinkedIn drafts via OpenRouter, stores them in Google Sheets for approval, then auto-publishes at 9 AM daily."
...
category: "Automation"
author: "Asif Hossain"
relatedService: "n8n-automation-australia"
coverImage: "https://ik.imagekit.io/zqm3n6ufa/Blogs/FINAL_OUTPUT.png?tr=w-1200,q-80"
faqs:
  - q: "Is n8n free to use?"
    a: "Yes. n8n is open-source and free if you self-host it on your own server. There is also a paid n8n Cloud tier if you would rather not manage hosting yourself. For this build, self-hosting keeps the whole system free apart from OpenRouter token costs."
  - q: "Why use OpenRouter instead of calling OpenAI or Anthropic directly?"
    a: "OpenRouter gives you dozens of models, GPT-4o, Claude, Llama, Mistral, Gemini, through one API key and one node. If a model gets expensive or its quality drops, you switch in a single dropdown with no new credentials and no reconfigured nodes."
  - q: "What is a community node in n8n?"
    a: "Community nodes are built by developers in the n8n community and published to npm, not maintained by the n8n team. You install them under Settings, Community Nodes. They unlock integrations like Upload Post, but quality varies, so check npm downloads and last-updated date first."
  - q: "Do I need to know how to code to build this?"
    a: "No. The whole system is built by connecting nodes visually in the n8n editor. The only structured part is defining a JSON shape for the AI output, and that is copy-paste. Planning each step with sticky notes first matters more than coding ability."
---
...
- A generation workflow that turns a one-line topic in Google Sheets into a finished LinkedIn post plus a matching AI image.
- A publishing workflow that checks the sheet every morning at 9 AM and posts only approved, scheduled rows to LinkedIn.
- A human review step in the middle, so nothing goes out until you change a row's status to "Approved."
...
Yes. n8n is open-source and free if you self-host it on your own server. There is also a paid n8n Cloud tier if you would rather not manage hosting yourself. For this build, self-hosting keeps the whole system free apart from OpenRouter token costs.

### Why use OpenRouter instead of calling OpenAI or Anthropic directly?

OpenRouter gives you dozens of models, GPT-4o, Claude, Llama, Mistral, Gemini, through one API key and one node. If a model gets expensive or its quality drops, you switch in a single dropdown with no new credentials and no reconfigured nodes.

### What is a community node in n8n?

Community nodes are built by developers in the n8n community and published to npm, not maintained by the n8n team. You install them under Settings, Community Nodes. They unlock integrations like Upload Post, but quality varies, so check npm downloads and last-updated date first.

### Do I need to know how to code to build this?

No. The whole system is built by connecting nodes visually in the n8n editor. The only structured part is defining a JSON shape for the AI output, and that is copy-paste. Planning each step with sticky notes first matters more than coding ability.
...
_Asif Hossain is a full-stack developer in Wollongong, NSW, building Next.js web apps and n8n automation systems for clients across Australia. This walkthrough is based on a system I built and run for my own LinkedIn posting._
